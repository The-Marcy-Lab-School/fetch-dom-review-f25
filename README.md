...
A live-coding review app that covers DOM manipulation, fetching with `.then()`/`.catch()`, fetching with `async`/`await`, ES Modules, event delegation, `dataset`, and form handling. The completed solution is in `src-solution/`.
...
Walk through the provided files before writing JavaScript:
...
- **`styles.css`** - Point out `.hidden { display: none !important; }`.
